...
void SphereSphereCollision01( const std::string input_filename, const std::string output_folder_name )
{
  ...
  auto particles = std::make_shared<
    CabanaDEM::Particles<memory_space, DIM>>(exec_space(), 2, output_folder_name);
  ...
  auto init_functor = KOKKOS_LAMBDA( const int pid )
	...
	x( pid, 0 ) = -radius_i + pid * (2. * radius_i) + pid * radius_i / 100.;
	x( pid, 1 ) = 0.;
	x( pid, 2 ) = 0.;
	u( pid, 0 ) = 10.0;
	if (pid == 1) u( pid, 0 ) = -10.0;
	u( pid, 1 ) = 0.0;
	u( pid, 2 ) = 0.0;
